 ...
 **/
	public class TimelineMax extends TimelineLite implements IEventDispatcher {
		...
		override public function renderTime($time:Number, $suppressEvents:Boolean=false, $force:Boolean=false):void {
			...
			var totalDur:Number = (this.cacheIsDirty) ? this.totalDuration : this.cachedTotalDuration, prevTime:Number = this.cachedTime, tween:Tweenable, isComplete:Boolean, rendered:Boolean, repeated:Boolean, next:Tweenable, isFirstRun:Boolean, dur:Number;
			if ($time >= totalDur) {
				...
				if (_cyclesComplete != int(this.cachedTotalTime / cycleDuration) && !isComplete && (this.cachedTime != prevTime || $force)) {
					...
					*/
					...
					if (forward) {
						forceChildrenToEnd(prevTime, $suppressEvents, false);
						if (!this.yoyo) {
							forceChildrenToBeginning(this.cachedDuration, $suppressEvents, true);
						}
					} else {
						forceChildrenToBeginning(prevTime, $suppressEvents, false);
						if (!this.yoyo) {
							forceChildrenToEnd(0, $suppressEvents, true);
						}
					}
				}
			}
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------------------------------------
		...
		override public function set time($n:Number):void {
			if (_cyclesComplete == 0) {
				...
				setTotalTime($n + (_cyclesComplete * this.duration), false);
			}
		}
