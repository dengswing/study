	...
	public class Demo5 extends Sprite
	{
		...
		private function cacheBitmapMC():void
		{
			...
			while (i < DemoConfig.ItemTypeNumber)
			{
				...
				BitmapFrameInfo.storeBitmapFrameInfo(String(i), BitmapCacher.cacheBitmapMovie(mc));
		...
		function initBitmapChilds(num:int):void
		{
			...
			{
				var child:VBitmapMovie = new VBitmapMovie(BitmapFrameInfo.getBitmapFrameInfo(String(Math.random() * DemoConfig.ItemTypeNumber ^ 0)));
		...
		function enterFrameHandler(evt:Event):void
		{
			...
			while (i < c)
			{
				var child:VBitmapMovie = arr_child[i];
