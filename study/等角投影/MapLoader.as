	...
	public class MapLoader extends EventDispatcher
	{
		...
		*/
		...
		private function parseDefinition(line:String):void
		{
			...
			// loop through the rest of the tokens, which are key/value pairs separated
			//by :
		...
		*/
		...
		/**
		* Creates an IsoWorld, iterates through loaded map, adding tiles to it based
		on map and definitions.
		...
		public function makeWorld(size:Number):IsoWorld
		{
			...
			for(var i:int = 0; i < _grid.length; i++)
			{
				for(var j:int = 0; j < _grid[i].length; j++)
				{
					...
					{
						case "DrawnIsoTile" :
							tile = new DrawnIsoTile(size, parseInt(cell.color),
							parseInt(cell.height));
							break;
						case "DrawnIsoBox" :
							tile = new DrawnIsoBox(size, parseInt(cell.color),
							parseInt(cell.height));
							break;
						case "GraphicTile" :
							var graphicClass:Class = getDefinitionByName(cell.graphicClass) as Class;							
							tile = new GraphicTile(size, graphicClass,
							parseInt(cell.xoffset), parseInt(cell.yoffset));
							break;
						default :
