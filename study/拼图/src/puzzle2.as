package 
{
	...
	public class puzzle2 extends BaseSprite
	{
										  ...
										  _rowNum:int = 8, _colNum:int = 8, _checkGap:int = 15):void
		{
			...
			for(i = 0; i < rowNum; i ++)
			{
				for(j = 0; j < colNum; j ++)
				{
					...
					{
						if(pArray[k].length>=3){
							chip.graphics.lineTo(pArray[k][0], pArray[k][1]);
						}else {
							//trace("***", pArray[k], "..", pArray[k + 1], "..", pArray[k + 2]);
							
							if(pArray[k][0]==pArray[k + 1][0])
								chip.graphics.curveTo(pArray[k][0], (pArray[k][1] + pArray[k + 2][1]) / 2, pArray[k + 2][0], pArray[k + 2][1]);
							else 
								chip.graphics.curveTo((pArray[k][0] + pArray[k + 1][0]) / 2, pArray[k][1], pArray[k + 2][0], pArray[k + 2][1]);
		...
		private final function configPoint(chip:Item_Chip):Array
		{
			...
			else
			{
				...
				tmp[0]						=	[tempArray[3][0] - this.chipWidth, tempArray[3][1], 1];
				tmp[1]						=	[tempArray[2][0] - this.chipWidth, tempArray[2][1]];
				tmp[2]						=	[tempArray[1][0] - this.chipWidth, tempArray[1][1]];
				tmp[3]						=	[tempArray[0][0] - this.chipWidth, tempArray[0][1], 1];
			...
			else
			{
				...
				tmp[0]						=	[tempArray[3][0], tempArray[3][1] - this.chipHeight, 1];
				tmp[1]						=	[tempArray[2][0], tempArray[2][1] - this.chipHeight];
				tmp[2]						=	[tempArray[1][0], tempArray[1][1] - this.chipHeight];
				tmp[3]						=	[tempArray[0][0], tempArray[0][1] - this.chipHeight, 1];
				chip.ATop					=	tmp.slice();
			}
